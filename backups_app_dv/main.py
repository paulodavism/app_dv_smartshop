...
def carregar_estoque_interno():
    ...
    try:
        with get_session() as db:
            # Consulta para obter os registros de estoque com informações do depósito e do produto
            statement = (
                select(Estoque, Deposito.nome.label("Depósito"), Produto.nome.label("Produto"))
                .join(Deposito, Estoque.deposito_id == Deposito.id)
                .join(Produto, Estoque.sku == Produto.sku)
                .where(Deposito.tipo == "Próprio", Estoque.quantidade > 0)  # Filtra apenas depósitos próprios com estoque
            )
            ...
            estoque_lista = [
                {
                    ...
                    "Estoque": int(registro.Estoque.quantidade)  # Garante formato inteiro
                }
    ...
    if not depositos:
        ...
        for idx, deposito in enumerate(depositos):
            ...
            with st.expander(deposito.nome, expanded=False):  # Sempre colapsado inicialmente
                ...
                cols = st.columns([1, 1, 3])  # Colunas para os botões
            ...
            if getattr(st.session_state, "deposito_para_excluir", None) == deposito.id:
                ...
                col_confirmacao = st.columns([1, 1])
...
def exibir_gestao_estoque():
    ...
    if menu_opcao == "Registrar Movimentação":
        ...
        if st.session_state.etapa == 1:
            ...
            with st.form("form_selecao_transferencia", clear_on_submit=False):
                produtos_selecionados = st.multiselect("Produtos*", options=list(produto_map.keys()), default=st.session_state.produtos_selecionados)
                origem_nome = st.selectbox("Origem*", options=list(deposito_map.keys()), index=list(deposito_map.keys()).index(origem_nome_default))
                destino_nome = st.selectbox("Destino*", options=[d for d in deposito_map.keys() if d != origem_nome], index=[d for d in deposito_map.keys() if d != origem_nome].index(destino_nome_default))
                if st.form_submit_button("Próxima Etapa"):
                    if not produtos_selecionados or not origem_nome or not destino_nome:
                        st.error("Todos os campos são obrigatórios.")
                    else:
                        st.session_state.produtos_selecionados = produtos_selecionados
        ...
        if st.button("Consultar Histórico"):
            sku = produto_map.get(produto_nome) if produto_nome != "Todos" else None
            deposito_id = deposito_map.get(deposito_nome) if deposito_nome != "Todos" else None
            ...
            logging.debug(f"SKU: {sku}, Deposito ID: {deposito_id}, Data Início: {data_inicio}, Data Fim: {data_fim}")
            ...
            try:
                ...
                if historico:
                    historico_formatado = []
                    for r in historico:
                        deposito_nome_from_id = next((d.nome for d in depositos if d.id == r.deposito_id), "Desconhecido")
                        historico_formatado.append({
                            "SKU": r.sku,
                            "Depósito": deposito_nome_from_id,
                            "Quantidade": r.quantidade,
                            "Tipo": r.tipo,
                            "Data/Hora": r.data_hora,
                            "Observações": r.observacoes,
                        })
                    st.table(historico_formatado)
                else:
                    st.info("Nenhuma movimentação encontrada.")
            except Exception as e:
                st.error(f"Erro ao consultar histórico: {str(e)}")
